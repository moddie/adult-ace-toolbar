        ...
        for ( $i = 1; $i <= $pages_count; $i++ ) {
            if ( abs($page - $i) < 5 ) {
                ...
                <li class="<?php echo ($i == $page) ? 'active' : '' ?>" ><a data-id="<?php echo $i ?>"
                   href="?page=<?php echo $i ?>&filter=<?php echo urlencode($filter) ?>"><?php echo $i ?></a></li>
                   <?php
               }
           }
           ?>
...
<?php if ( $page > 1 ) { ?>
            jQuery('.prev').attr('href', '?page=<?php echo ($page - 1) ?>&filter=<?php echo urlencode($filter) ?>');
<?php } ?>
<?php if ( $page < $pages_count ) { ?>
            jQuery('.next').attr('href', '?page=<?php echo ($page + 1) ?>&filter=<?php echo urlencode($filter) ?>');
<?php } ?>
    });
</script>